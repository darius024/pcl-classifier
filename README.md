...
The submitted `dev.txt` and `test.txt` can be regenerated by running **`BestModel/model.ipynb`** from top to bottom.

**Default mode (fast — loads saved weights):**

The notebook defaults to `TRAIN_MODE = False`, which skips retraining and loads the saved checkpoints directly from `BestModel/roberta-base/` and `BestModel/distilbert/`. Running all cells will:
