...
\textbf{Submission summary.} The \texttt{BestModel/} folder in the repository holds the saved checkpoints for both backbones. The submission files \texttt{dev.txt} (2,094 lines) and \texttt{test.txt} (3,832 lines) are in the repository root, each containing one prediction per line (0 = No PCL, 1 = PCL). The ensemble achieves \textbf{F1 = 0.6069} on the official dev set, surpassing the baseline of 0.48 by $+$0.1269. Table~\ref{tab:final_results} summarises the final system comparison.
...
Official baseline (RoBERTa-base)          & --- & 0.48 & --- \\
\bottomrule
\end{tabular}
\caption{Final system comparison on the official dev set.}
...
\subsection{Additional Local Evaluation: PR Curve, Multi-Seed Analysis, and Ablation Study}
...
The AUC-PR of \textbf{0.5906} is substantially higher than the no-skill baseline of $\approx$0.095 (equal to the PCL rate), confirming real discriminative power. Key trade-offs along the curve:
\begin{itemize}[noitemsep]
    \item Recall $>$0.85 is achievable at the cost of precision $\sim$0.25--0.30, preferable when missing PCL is costly (e.g., large-scale content auditing).
    \item Threshold 0.75 gives precision $>$0.70 at recall $\approx$0.50, preferable when false positives carry a high cost.
    \item Our chosen threshold of 0.50 (precision = 0.560, recall = 0.653) maximises F1 in line with the SemEval criterion.
\end{itemize}
...
\caption{RoBERTa-base (drop=0.0, bs=4, ga=4) across three random seeds; bar heights show best-threshold F1 on the dev set. Whiskers (if present) show AUC-PR.}
...
Training the same architecture with three different seeds yields F1 values spanning 0.569--0.603 and optimal thresholds varying from 0.12 to 0.87. This reveals a well-known instability in fine-tuning large transformers on small, imbalanced datasets: minor differences in mini-batch ordering early in training can shift the model towards different local minima, each requiring a different decision boundary. The large threshold spread further underscores that defaulting to $t=0.5$ would be sub-optimal for at least two of the three seeds. That \textbf{seed~7 generalises best to the held-out dev set} (0.6034) despite not always having the highest internal validation score shows that validation metrics are only a noisy proxy for true test performance. This variance directly motivates ensembling: averaging probabilities over multiple seeds and architectures smooths out seed-specific biases and produces a more calibrated score distribution, resulting in the ensemble's improved dev F1 of 0.6069.

\subsubsection*{Component Ablation Study}

A rigorous component ablation was conducted to isolate the contribution of each training-time design decision. Using \texttt{distilbert-base-uncased} (seed=42) as a controlled backbone, we re-trained with exactly one component removed per variant; all other hyperparameters remained fixed at the full-model values. The five components tested are:

\begin{enumerate}[noitemsep]
    \item \textbf{Metadata prepending}: remove the \texttt{<e>keyword</e> <e>country</e>} prefix; pass raw article text only.
    \item \textbf{Weighted Random Sampler (WRS)}: replace with a standard uniform DataLoader.
    \item \textbf{Grouped LLRD}: replace with a single uniform learning rate (2e-5) across all layers.
    \item \textbf{Linear warmup}: remove the 6\% warmup phase; start cosine decay from step 0.
    \item \textbf{Threshold tuning}: fix decision threshold at $t=0.50$ instead of sweeping on the validation set.
\end{enumerate}

All variants are trained for up to 10 epochs with early stopping (patience=3) and evaluated on the official dev set. Full code and logs are in \texttt{5\_evaluation/ablation\_study.ipynb}.

\begin{table}[H]
\centering
\begin{tabular}{lcccc}
\toprule
Variant & Val F1 & Test F1 & $\Delta$ F1 & Best epoch \\
\midrule
Full model (all components ON) & \textit{fill in} & \textit{fill in} & 0.000 & \textit{fill in} \\
w/o Metadata prepending        & \textit{fill in} & \textit{fill in} & \textit{fill in} & \textit{fill in} \\
w/o Weighted Random Sampler    & \textit{fill in} & \textit{fill in} & \textit{fill in} & \textit{fill in} \\
w/o Grouped LLRD               & \textit{fill in} & \textit{fill in} & \textit{fill in} & \textit{fill in} \\
w/o Linear warmup              & \textit{fill in} & \textit{fill in} & \textit{fill in} & \textit{fill in} \\
Fixed threshold $t=0.50$       & \textit{fill in} & \textit{fill in} & \textit{fill in} & \textit{fill in} \\
\bottomrule
\end{tabular}
\caption{Component-level ablation results (DistilBERT, seed=42). $\Delta$F1 = test F1 minus full-model F1; negative means that component is helpful. \textit{Run \texttt{5\_evaluation/ablation\_study.ipynb} and update.}}
\label{tab:ablation_component}
\end{table}

\textbf{Part B: Dropout and batch-size ablation} (from the full hyperparameter search; results are the best validation F1 across three seeds per configuration):
...
\bottomrule
\end{tabular}
\caption{Hyperparameter search results: best-seed validation F1. Bold marks the best seed per row. \textit{bs}=batch size, \textit{ga}=gradient accumulation steps (effective batch = bs\,$\times$\,ga).}
\label{tab:ablation_dropout}
\end{table}

Three findings emerge from Table~\ref{tab:ablation_dropout}:

\textbf{1. Classifier-head dropout is the most destructive hyperparameter.} RoBERTa with drop=0.5 collapses from val F1~$\sim$0.598 down to $\sim$0.28--0.32, a drop of over 0.30 F1 points. This is not a minor degradation; it renders the model nearly useless. The explanation is straightforward: with only 794 positive training examples, the model needs every gradient signal to adapt the classifier head rapidly to a highly skewed distribution, and a 50\% dropout rate effectively halves the available gradient per step. Drop=0.0 (no extra dropout beyond the pre-trained layers themselves) is the right choice for RoBERTa. DistilBERT is somewhat more tolerant of modest dropout (0.1--0.3) due to its smaller capacity and the fact that its pre-trained layers already apply less dropout.

\textbf{2. Batch size and gradient accumulation are largely irrelevant.} For RoBERTa drop=0.0, both batch configurations (bs=8/ga=2 and bs=4/ga=4, both effective batch=16) peak at an identical best-seed val F1 of 0.5979. The same pattern holds for DistilBERT. Given that the effective batch size is equal in both cases, this confirms that gradient quality (signal-to-noise ratio per update) matters, not the number of updates per epoch.

\textbf{3. Ensembling provides the largest gain over any individual checkpoint.} The best solo checkpoint (RoBERTa, val F1=0.5979, dev F1=0.6034) does not match the ensemble (dev F1=0.6069). The $+$0.004 gain on the dev set (after threshold tuning per model) confirms that RoBERTa and DistilBERT make partially uncorrelated errors, as the Venn-style comparison in Figure~\ref{fig:rvd} directly shows.

\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{images/roberta_vs_distilbert.png}
\caption{Error overlap between the best RoBERTa and DistilBERT checkpoints on the dev set. Cases where only one model errs are the primary motivation for ensembling.}
\label{fig:rvd}
\end{figure}
